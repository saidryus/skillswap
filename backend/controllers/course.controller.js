...
const createCourse = async (req, res) => {
  try {
    const { courseCode, courseName, description, units, department, faculty } = req.body;

    const exists = await Course.findOne({ courseCode });
    if (exists) return res.status(400).json({ message: 'Course code already exists' });

    const course = await Course.create({ courseCode, courseName, description, units, department, faculty });
