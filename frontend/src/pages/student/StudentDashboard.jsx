  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${user.studentIdNumber || 'College of Computer Studies'} · Year ${user.yearLevel || '—'}`}
      />

      {/* Operational stat cards — what needs attention right now */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard title="Upcoming Sessions" value={upcoming.length} icon={HiCalendar} color="blue" />
        <StatCard title="Awaiting Acceptance" value={pending} icon={HiClock} color="amber" />
        <StatCard title="Tutor Courses" value={approvedCourses} icon={HiStar} color="purple" />
        <StatCard title="Pending Applications" value={pendingApps} icon={HiBookOpen} color="orange" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="card">
          <h2 className="text-lg font-bold text-surface-900 dark:text-white mb-4">Upcoming Sessions</h2>
          {upcoming.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <button onClick={() => navigate('/student/find-tutor')}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card">
          <h2 className="text-lg font-bold text-surface-900 dark:text-white mb-4">Announcements</h2>
              ...
              {announcements.map((a, idx) => (
                <motion.div key={a._id}
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <p className="text-sm font-semibold text-surface-800 dark:text-surface-100">{a.title}</p>
                    {a.isPinned && <span className="badge-warning shrink-0 text-[10px]">Pinned</span>}
                  </div>
                  <p className="text-xs text-surface-500 dark:text-surface-400 mt-1.5 line-clamp-2">{a.content}</p>
